...
回答:""")
    ...
    def generate_step_by_step_answer(self, query: str, context_docs: List[Document]) -> str:
        ...
        prompt = ChatPromptTemplate.from_template("""
你是一位专业的烹饪导师。请根据食谱信息{context}，为用户提供详细的分步骤指导。
...
注意：
- 你的回答必须 100% 基于相关食谱信息{context}。严禁加入任何你自己的烹饪经验或外部知识，如果文档里没写，就说不知道，不准凭经验发挥。并在回复开头添加“文档不存在相关内容”
- 根据实际内容灵活调整结构
...
重写原则：
- 保持原意不变：核心诉求（如想吃辣、想喝饮料）必须保留。
- 增加相关烹饪术语：将模糊词转化为检索关键词（如“做法” 、“菜谱”、“步骤”）。
- 优先推荐简单易做的：在重写中加入“简单”、“家常”等限定词，提高用户体验。
- 保持简洁性：重写结果应为短语，不建议输出长难句。
...
请提供详细、实用的回答。如果文档{context}里没写，说不知道，不准凭经验发挥。并在回复开头添加“文档不存在相关内容”
...
回答:""")
    ...
    def generate_step_by_step_answer_stream(self, query: str, context_docs: List[Document]):
        ...
        prompt = ChatPromptTemplate.from_template("""
你是一位专业的烹饪导师。请根据食谱信息{context}，为用户提供详细的分步骤指导，如果文档里没写，就说不知道，不准凭经验发挥。
...
注意：
- 你的回答必须 100% 基于相关食谱信息{context}。严禁加入任何你自己的烹饪经验或外部知识，如果文档里没写，就说不知道，不准凭经验发挥。
- 根据实际内容灵活调整结构
- 不要强行填充无关内容
- 重点突出实用性和可操作性
